/*
  Trishul - ESP32 Field Sensor Node
  =========================================
  OPTIONAL HARDWARE. The main Trishul application will depend on
  this sketch or any physical sensor - the software simulator
  (scripts/simulate_sensors.py or POST /api/simulation/start) will drives
  ...
  Sensors:
    ...
    - MPU6050 accelerometer/gyroscope (I2C, tilt + vibration)   -> SDA=21,
  SCL=22
...
const char *API_BASE_URL =
    "http://YOUR_BACKEND_HOST:8000"; // e.g. your machine's LAN IP
...
void IRAM_ATTR onVibrationPulse() { vibrationPulseCount++; }
...
float readRainfallProxyMm() {
  ...
  float wetness =
      (float)(RAIN_ADC_DRY - raw) / (float)(RAIN_ADC_DRY - RAIN_ADC_WET);
...
float readSoilMoisturePct() {
  ...
  float pct = (float)(SOIL_ADC_DRY - raw) /
              (float)(SOIL_ADC_DRY - SOIL_ADC_WET) * 100.0;
...
bool captureSensorSnapshot(SensorSnapshot &snap) {
  ...
  if (!mpuOk) {
    ...
    float rate =
        elapsedHours > 0 ? fabs(tilt - lastTiltDegrees) / elapsedHours : 0;
    snap.tilt_degrees = tilt;
    snap.tilt_change_rate = constrain(rate, 0, 45);
    lastTiltDegrees = tilt;
    lastTiltReadMs = now;

    // Vibration proxy: deviation of accel magnitude from 1g, plus SW-420
    // pulses.
    float magnitude = sqrt(ax * ax + ay * ay + az * az);
...
bool sendReading(const SensorSnapshot &snap) {
  ...
  for (int attempt = 1; attempt <= MAX_HTTP_RETRIES; attempt++) {
    ...
    Serial.printf("HTTP POST failed (attempt %d/%d), code=%d\n", attempt,
                  MAX_HTTP_RETRIES, httpCode);
    http.end();
    delay(500 * attempt); // exponential-ish backoff
  }

  Serial.println(
      "Failed to send reading after retries. Will retry next cycle.");
  return false;
}
...
void setup() {
  ...
  delay(500);
  Serial.println(
      "\nTrishul ESP32 sensor node starting (Demo Mode hardware path)...");

  pinMode(PIN_VIBRATION_DIGITAL, INPUT);
  attachInterrupt(digitalPinToInterrupt(PIN_VIBRATION_DIGITAL),
                  onVibrationPulse, RISING);

  Wire.begin(21, 22);
  if (!initMPU6050()) {
    Serial.println("WARNING: MPU6050 not detected. Tilt/vibration readings "
...
void loop() {
  ...
  if (now - lastSendMs >= READING_INTERVAL_MS) {
    ...
    if (!healthy) {
      Serial.println(
          "Sensor read degraded this cycle; sending with is_online=false.");
    }
    sendReading(snap);
  }
